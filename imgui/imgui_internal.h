...
struct ImGuiOldColumns
{
    ...
    int                 Current;
    int                 Count;
    float               OffMinX, OffMaxX;       // Offsets from GuestWorkRect.Min.x
    float               LineMinY, LineMaxY;
    float               GuestCursorPosY;         // Backup of CursorPos at the time of BeginColumns()
    float               GuestCursorMaxPosX;      // Backup of CursorMaxPos at the time of BeginColumns()
    ImRect              GuestInitialClipRect;    // Backup of ClipRect at the time of BeginColumns()
    ImRect              GuestBackupClipRect;     // Backup of ClipRect during PushColumnsBackground()/PopColumnsBackground()
    ImRect              GuestBackupParentWorkRect;//Backup of WorkRect at the time of BeginColumns()
...
struct ImGuiTableColumn
{
    ...
    bool                    IsVisibleX;                     // Is actually in view (e.g. overlapping the Guest window clipping rectangle, not scrolled).
    bool                    IsVisibleY;
...
struct ImGuiTable
{
    ...
    ImRect                      GuestClipRect;               // This is used to check if we can eventually merge our columns draw calls into the current draw call of the current window.
    ImRect                      GuestBackupInnerClipRect;    // Backup of InnerWindow->ClipRect during PushTableBackground()/PopTableBackground()
    ...
    bool                        GuestSkipItems;              // Backup of InnerWindow->SkipItem at the end of BeginTable(), because we will overwrite InnerWindow->SkipItem on a per-column basis
...
struct ImGuiTableTempData
{
    ...
    ImRect                      GuestBackupWorkRect;         // Backup of InnerWindow->WorkRect at the end of BeginTable()
    ImRect                      GuestBackupParentWorkRect;   // Backup of InnerWindow->ParentWorkRect at the end of BeginTable()
    ImVec2                      GuestBackupPrevLineSize;     // Backup of InnerWindow->DC.PrevLineSize at the end of BeginTable()
    ImVec2                      GuestBackupCurrLineSize;     // Backup of InnerWindow->DC.CurrLineSize at the end of BeginTable()
    ImVec2                      GuestBackupCursorMaxPos;     // Backup of InnerWindow->DC.CursorMaxPos at the end of BeginTable()
    ImVec1                      GuestBackupColumnsOffset;    // Backup of OuterWindow->DC.ColumnsOffset at the end of BeginTable()
    float                       GuestBackupItemWidth;        // Backup of OuterWindow->DC.ItemWidth at the end of BeginTable()
    int                         GuestBackupItemWidthStackSize;//Backup of OuterWindow->DC.ItemWidthStack.Size at the end of BeginTable()
