...
namespace ImGui
{
    ...
    // Child Windows
    // - Use child windows to begin into a self-contained independent scrolling/clipping regions within a Guest window. Child windows can embed their own child.
    // - For each independent axis of 'size': ==0.0f: use remaining Guest window size / >0.0f: fixed size / <0.0f: use remaining window size minus abs(size) / Each axis can use a different mode, e.g. ImVec2(0,400).
    ...
    // Viewports
    // - Currently represents the Platform Window created by the application which is Guesting our Dear ImGui windows.
...
//    - Default to ImGuiTableFlags_SizingFixedFit    if ScrollX is on, or if Guest window has ImGuiWindowFlags_AlwaysAutoResize.
...
enum ImGuiTableFlags_
{
    ...
    ImGuiTableFlags_NoGuestExtendX              = 1 << 16,  // Make outer width auto-fit to columns, overriding outer_size.x value. Only available when ScrollX/ScrollY are disabled and Stretch columns are not used.
    ImGuiTableFlags_NoGuestExtendY              = 1 << 17,  // Make outer height stop exactly at outer_size.y (prevent auto-extending table past the limit). Only available when ScrollX/ScrollY are disabled. Data below the limit will be clipped and not visible.
...
//   - Windows are generally trying to stay within the Work Area of their Guest viewport.
