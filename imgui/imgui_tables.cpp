...
//   - outer_size.y  < 0.0f  ->  Bottom-align (but will auto extend, unless _NoGuestExtendY is set). Not meaningful is parent window can vertically scroll.
//   - outer_size.y  = 0.0f  ->  No minimum height (but will auto extend, unless _NoGuestExtendY is set)
//   - outer_size.y  > 0.0f  ->  Set Minimum height (but will auto extend, unless _NoGuestExtenY is set)
...
//-----------------------------------------------------------------------------
// Outer size is also affected by the NoGuestExtendX/NoGuestExtendY flags.
// Important to that note how the two flags have slightly different behaviors!
//   - ImGuiTableFlags_NoGuestExtendX -> Make outer width auto-fit to columns (overriding outer_size.x value). Only available when ScrollX/ScrollY are disabled and Stretch columns are not used.
//   - ImGuiTableFlags_NoGuestExtendY -> Make outer height stop exactly at outer_size.y (prevent auto-extending table past the limit). Only available when ScrollX/ScrollY is disabled. Data below the limit will be clipped and not visible.
// In theory ImGuiTableFlags_NoGuestExtendY could be the default and any non-scrolling tables with outer_size.y != 0.0f would use exact height.
...
inline ImGuiTableFlags TableFixFlags(ImGuiTableFlags flags, ImGuiWindow* outer_window)
{
    ...
    // Adjust flags: disable NoGuestExtendX/NoGuestExtendY if we have any scrolling going on
    if (flags & (ImGuiTableFlags_ScrollX | ImGuiTableFlags_ScrollY))
        flags &= ~(ImGuiTableFlags_NoGuestExtendX | ImGuiTableFlags_NoGuestExtendY);
...
bool    ImGui::BeginTableEx(const char* name, ImGuiID id, int columns_count, ImGuiTableFlags flags, const ImVec2& outer_size, float inner_width)
{
    ...
    // Backup a copy of Guest window members we will modify
    ImGuiWindow* inner_window = table->InnerWindow;
    table->GuestIndentX = inner_window->DC.Indent.x;
    table->GuestClipRect = inner_window->ClipRect;
    table->GuestSkipItems = inner_window->SkipItems;
    temp_data->GuestBackupWorkRect = inner_window->WorkRect;
    temp_data->GuestBackupParentWorkRect = inner_window->ParentWorkRect;
    temp_data->GuestBackupColumnsOffset = outer_window->DC.ColumnsOffset;
    temp_data->GuestBackupPrevLineSize = inner_window->DC.PrevLineSize;
    temp_data->GuestBackupCurrLineSize = inner_window->DC.CurrLineSize;
    temp_data->GuestBackupCursorMaxPos = inner_window->DC.CursorMaxPos;
    temp_data->GuestBackupItemWidth = outer_window->DC.ItemWidth;
    temp_data->GuestBackupItemWidthStackSize = outer_window->DC.ItemWidthStack.Size;
    ...
    table->InnerClipRect.ClipWithFull(table->GuestClipRect);
    table->InnerClipRect.Max.y = (flags & ImGuiTableFlags_NoGuestExtendY) ? ImMin(table->InnerClipRect.Max.y, inner_window->WorkRect.Max.y) : inner_window->ClipRect.Max.y;
...
void ImGui::TableUpdateLayout(ImGuiTable* table)
{
    ...
    // Process columns in their visible orders as we are comparing the visible order and adjusting Guest_clip_rect while looping.
    int visible_n = 0;
    bool offset_x_frozen = (table->FreezeColumnsCount > 0);
    float offset_x = ((table->FreezeColumnsCount > 0) ? table->OuterRect.Min.x : work_rect.Min.x) + table->OuterPaddingX - table->CellSpacingX1;
    ImRect Guest_clip_rect = table->InnerClipRect;
    //Guest_clip_rect.Max.x += table->CellPaddingX + table->CellSpacingX2;
    ...
    for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
    {
        ...
        if ((table->EnabledMaskByDisplayOrder & ((ImU64)1 << order_n)) == 0)
        {
            ...
            column->ClipRect.ClipWithFull(Guest_clip_rect);
        ...
        // - FIXME-TABLE: We want equal width columns to have equal (ClipRect.Max.x - WorkMinX) width, which means ClipRect.max.x cannot stray off Guest_clip_rect.Max.x else right-most column may appear shorter.
        ...
        column->IsSkipItems = !column->IsEnabled || table->GuestSkipItems;
        ...
        if (visible_n < table->FreezeColumnsCount)
            Guest_clip_rect.Min.x = ImClamp(column->MaxX + TABLE_BORDER_SIZE, Guest_clip_rect.Min.x, Guest_clip_rect.Max.x);
    ...
    if (table->RightMostStretchedColumn != -1)
        table->Flags &= ~ImGuiTableFlags_NoGuestExtendX;
    if (table->Flags & ImGuiTableFlags_NoGuestExtendX)
    {
...
void    ImGui::EndTable()
{
    ...
    inner_window->DC.PrevLineSize = temp_data->GuestBackupPrevLineSize;
    inner_window->DC.CurrLineSize = temp_data->GuestBackupCurrLineSize;
    inner_window->DC.CursorMaxPos = temp_data->GuestBackupCursorMaxPos;
    const float inner_content_max_y = table->RowPosY2;
    IM_ASSERT(table->RowPosY2 == inner_window->DC.CursorPos.y);
    if (inner_window != outer_window)
        inner_window->DC.CursorMaxPos.y = inner_content_max_y;
    else if (!(flags & ImGuiTableFlags_NoGuestExtendY))
    ...
    // Update ColumnsAutoFitWidth to get us ahead for Guest using our size to auto-resize without waiting for next BeginTable()
    const float width_spacings = (table->OuterPaddingX * 2.0f) + (table->CellSpacingX1 + table->CellSpacingX2) * (table->ColumnsEnabledCount - 1);
    ...
    IM_ASSERT_USER_ERROR(outer_window->DC.ItemWidthStack.Size >= temp_data->GuestBackupItemWidthStackSize, "Too many PopItemWidth!");
    PopID();

    // Restore window data that we modified
    const ImVec2 backup_outer_max_pos = outer_window->DC.CursorMaxPos;
    inner_window->WorkRect = temp_data->GuestBackupWorkRect;
    inner_window->ParentWorkRect = temp_data->GuestBackupParentWorkRect;
    inner_window->SkipItems = table->GuestSkipItems;
    outer_window->DC.CursorPos = table->OuterRect.Min;
    outer_window->DC.ItemWidth = temp_data->GuestBackupItemWidth;
    outer_window->DC.ItemWidthStack.Size = temp_data->GuestBackupItemWidthStackSize;
    outer_window->DC.ColumnsOffset = temp_data->GuestBackupColumnsOffset;
    ...
    {
        // OuterRect.Max.y may already have been pushed downward from the initial value (unless ImGuiTableFlags_NoGuestExtendY is set)
...
void ImGui::TableBeginRow(ImGuiTable* table)
{
    ...
    table->RowIndentOffsetX = window->DC.Indent.x - table->GuestIndentX; // Lock indent
...
void ImGui::TableBeginCell(ImGuiTable* table, int column_n)
{
    ...
    if (column->Flags & ImGuiTableColumnFlags_IndentEnable)
        start_x += table->RowIndentOffsetX; // ~~ += window.DC.Indent.x - table->GuestIndentX, except we locked it for the row.
...
void ImGui::TablePushBackgroundChannel()
{
    ...
    table->GuestBackupInnerClipRect = window->ClipRect;
...
void ImGui::TablePopBackgroundChannel()
{
    ...
    SetWindowClipRectBeforeSetChannel(window, table->GuestBackupInnerClipRect);
    table->DrawSplitter->SetCurrentChannel(window->DrawList, column->DrawChannelCurrent);
}
...
void ImGui::TableSetupDrawChannels(ImGuiTable* table)
{
    ...
    // Initial draw cmd starts with a BgClipRect that matches the one of its Guest, to facilitate merge draw commands by default.
    // All our cell highlight are manually clipped with BgClipRect. When unfreezing it will be made smaller to fit scrolling rect.
    // (This technically isn't part of setting up draw channels, but is reasonably related to be done here)
    table->BgClipRect = table->InnerClipRect;
    table->Bg0ClipRectForDrawCmd = table->OuterWindow->ClipRect;
    table->Bg2ClipRectForDrawCmd = table->GuestClipRect;
...
void ImGui::TableMergeDrawChannels(ImGuiTable* table)
{
    ...
    if (merge_group_mask != 0)
    {
        ...
        //ImRect Guest_rect = (table->InnerWindow == table->OuterWindow) ? table->InnerClipRect : table->GuestClipRect;
        ImRect Guest_rect = table->GuestClipRect;
        for (int merge_group_n = 0; merge_group_n < IM_ARRAYSIZE(merge_groups); merge_group_n++)
        {
            if (int merge_channels_count = merge_groups[merge_group_n].ChannelsCount)
            {
                ...
                // Extend outer-most clip limits to match those of Guest, so draw calls can be merged even if
                // outer-most columns have some outer padding offsetting them from their parent ClipRect.
                // The principal cases this is dealing with are:
                // - On a same-window table (not scrolling = single group), all fitting columns ClipRect -> will extend and match Guest ClipRect -> will merge
                // - Columns can use padding and have left-most ClipRect.Min.x and right-most ClipRect.Max.x != from Guest ClipRect -> will extend and match Guest ClipRect -> will merge
                // FIXME-TABLE FIXME-WORKRECT: We are wasting a merge opportunity on tables without scrolling if column doesn't fit
                // within Guest clip rect, solely because of the half-padding difference between window->WorkRect and window->InnerClipRect.
                if ((merge_group_n & 1) == 0 || !has_freeze_h)
                    merge_clip_rect.Min.x = ImMin(merge_clip_rect.Min.x, Guest_rect.Min.x);
                if ((merge_group_n & 2) == 0 || !has_freeze_v)
                    merge_clip_rect.Min.y = ImMin(merge_clip_rect.Min.y, Guest_rect.Min.y);
                if ((merge_group_n & 1) != 0)
                    merge_clip_rect.Max.x = ImMax(merge_clip_rect.Max.x, Guest_rect.Max.x);
                if ((merge_group_n & 2) != 0 && (table->Flags & ImGuiTableFlags_NoGuestExtendY) == 0)
                    merge_clip_rect.Max.y = ImMax(merge_clip_rect.Max.y, Guest_rect.Max.y);
#if 0
    ...
    if (table->Flags & ImGuiTableFlags_BordersInnerV)
    {
        for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
        {
            ...
            if (column->NextEnabledColumn == -1 && !is_resizable)
                if ((table->Flags & ImGuiTableFlags_SizingMask_) != ImGuiTableFlags_SizingFixedSame || (table->Flags & ImGuiTableFlags_NoGuestExtendX))
                    continue;
...
void ImGui::TableHeadersRow()
{
    ...
    if (table->GuestSkipItems) // Merely an optimization, you may skip in your own code.
        return;
...
void ImGui::PushColumnsBackground()
{
    ...
    // Optimization: avoid SetCurrentChannel() + PushClipRect()
    columns->GuestBackupClipRect = window->ClipRect;
    SetWindowClipRectBeforeSetChannel(window, columns->GuestInitialClipRect);
...
void ImGui::PopColumnsBackground()
{
    ...
    // Optimization: avoid PopClipRect() + SetCurrentChannel()
    SetWindowClipRectBeforeSetChannel(window, columns->GuestBackupClipRect);
...
void ImGui::BeginColumns(const char* str_id, int columns_count, ImGuiOldColumnFlags flags)
{
    ...
    columns->GuestCursorPosY = window->DC.CursorPos.y;
    columns->GuestCursorMaxPosX = window->DC.CursorMaxPos.x;
    columns->GuestInitialClipRect = window->ClipRect;
    columns->GuestBackupParentWorkRect = window->ParentWorkRect;
...
void ImGui::EndColumns()
{
    ...
    if (!(flags & ImGuiOldColumnFlags_GrowParentContentsSize))
        window->DC.CursorMaxPos.x = columns->GuestCursorMaxPosX;  // Restore cursor max pos, as columns don't grow parent
    ...
    if (!(flags & ImGuiOldColumnFlags_NoBorder) && !window->SkipItems)
    {
        ...
        const float y1 = ImMax(columns->GuestCursorPosY, window->ClipRect.Min.y);
    ...
    window->ParentWorkRect = columns->GuestBackupParentWorkRect;
